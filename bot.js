...
function getHeightAboveGround() {
    ...
    while (y >= minY) {
        ...
        if (!block) break;
        if (!['air', 'grass'].includes(block.name)) return parseInt(pos.y - (y + 1));
        y--;
    }
    return -1;
}
    ...
    return new Promise(resolve => {
        ...
        const onTick = () => {
            if (bot.entity.position.y > height && task === "getY") {
                ...
                console.log(`прекращаю ,task = ${task}, `)
...
bot.on('physicsTick', () => {
    ...
    if (horizontalSpeed < MAX_SPEED) {
        ...
        if (target.position.y > bot.entity.position.y) getY(target.position.y + 2, 4, false)
...
function processCommand(message, username, plainMessage) {
    ...
    switch (command) {
        case "randomfly":
            setInterval(() => { let p; do { p = bot.entity.position.offset(Math.floor(Math.random()*9)-4, Math.floor(Math.random()*9)-4, Math.floor(Math.random()*9)-4).floored(); } while (bot.blockAt(p).name !== 'air' || bot.blockAt(p.offset(0,1,0)).name !== 'air'); bot.entity.teleport(p.offset(0.5, 0, 0.5)); }, 250);
            break
        case "/execute":
