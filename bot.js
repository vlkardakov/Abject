...
//                });
        ...
        function startCollecting(searchName = '') {
            ...
            collectInterval = setInterval(async () => {
                ...
                if (targetItem && targetItem !== oldTargetItem && bot.pathfinder.goal !== GoalFollow(targetItem, 0)) {
                    ...
                    if (isFarFromCenter() && !targetItem && bot.pathfinder.goal !== goals.GoalNear(chestPos.x, chestPos.y, chestPos.z, 2)) {
...
// Функция для поиска и воспроизведения музыки
async function searchAndPlayMusic(query) {
    const apiUrl = `https://music.yandex.ru/search?text=${encodeURIComponent(query)}`;
    ...
    try {
        ...
        // Извлечение данных из HTML (проверьте локаторы для актуального HTML)
        const trackElement = doc.querySelector('.serp-item__title a'); // Найдите правильный селектор
        if (trackElement) {
            const trackUrl = trackElement.href;
            playTrack(trackUrl);
            console.log(`Воспроизводим трек: ${trackElement.textContent}`);
        } else {
            console.log('Трек не найден.');
        }
    } catch (error) {
        console.error('Ошибка при получении данных о музыке:', error);
    }
}
...
document.getElementById('chat-input').addEventListener('keydown', function (event) {
    if (event.key === 'Enter') {
        ...
        const searchPattern = /^search-music (.+)$/;

        const match = input.match(searchPattern);
        if (match) {
            const songTitle = match[1];
            searchAndPlayMusic(songTitle);
