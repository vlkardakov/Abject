...
function processCommand(message, username, plainMessage) {
    ...
    switch (command) {
        ...
        case "say":
            ...
            return;
        case "activate": if (args.length < 1) return; const target = findEntityWithName(bot, args[0]); if (target) bot.lookAt(target.position.offset(0, target.height * 0.9, 0)).then(() => { bot.swingArm(); bot._client.write('use_entity', { target: target.id, type: 0, hand: 0 }); }); return;
